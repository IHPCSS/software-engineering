...
TEST(ConstantBCTest, LeftAndRightEdges)
{
    ...
    for (std::size_t j = 1; j < 3; ++j) {
        EXPECT_DOUBLE_EQ(grid(0,      j), 1.0) << "left  j=" << j;
        EXPECT_DOUBLE_EQ(grid(4,      j), 2.0) << "right j=" << j;
    }
}
...
TEST(ConstantBCTest, BottomAndTopEdges)
{
    ...
    for (std::size_t i = 0; i < 5; ++i) {
        EXPECT_DOUBLE_EQ(grid(i, 0), 20.0) << "bottom i=" << i;
        EXPECT_DOUBLE_EQ(grid(i, 3), 10.0) << "top    i=" << i;
    }
}
...
TEST(ConstantBCTest, CornersOwnedByTopAndBottom)
{
    ...
    EXPECT_DOUBLE_EQ(grid(0, 0), 20.0); // bottom-left corner  → bottom
    EXPECT_DOUBLE_EQ(grid(4, 0), 20.0); // bottom-right corner → bottom
    EXPECT_DOUBLE_EQ(grid(0, 3), 10.0); // top-left corner     → top
    EXPECT_DOUBLE_EQ(grid(4, 3), 10.0); // top-right corner    → top
}
...
TEST(ConstantBCTest, UniformBC)
{
    ...
    for (std::size_t i = 0; i < 4; ++i) {
        EXPECT_DOUBLE_EQ(grid(i, 0), 5.0);
        EXPECT_DOUBLE_EQ(grid(i, 3), 5.0);
    }
    for (std::size_t j = 0; j < 4; ++j) {
        EXPECT_DOUBLE_EQ(grid(0, j), 5.0);
        EXPECT_DOUBLE_EQ(grid(3, j), 5.0);
    }
    // Interior cells must not be touched.
    EXPECT_DOUBLE_EQ(grid(1, 1), 0.0);
    EXPECT_DOUBLE_EQ(grid(2, 2), 0.0);
}
...
TEST(CornerHeatBCTest, LeftEdgeIsZero)
{
    ...
    for (std::size_t j = 0; j < 3; ++j) {
        EXPECT_DOUBLE_EQ(grid(0, j), 0.0) << "left j=" << j;
    }
}
...
TEST(CornerHeatBCTest, TopEdgeIsZero)
{
    ...
    for (std::size_t i = 0; i < 3; ++i) {
        EXPECT_DOUBLE_EQ(grid(i, 2), 0.0) << "top i=" << i;
    }
}
...
TEST(CornerHeatBCTest, BottomEdgeRamp)
{
    ...
    EXPECT_DOUBLE_EQ(grid(1, 0),  50.0);
    EXPECT_DOUBLE_EQ(grid(2, 0), 100.0);
}
...
TEST(CornerHeatBCTest, RightEdgeInteriorRamp)
{
    ...
    // j=1: (100 / (ny-1=2)) * 1 = 50.
    EXPECT_DOUBLE_EQ(grid(2, 1), 50.0);
}
...
TEST(CornerHeatBCTest, InteriorUntouched)
{
    ...
    EXPECT_DOUBLE_EQ(grid(1, 1), 99.0);
}
